...
if [ ! -d "$PHOENIXD_DATA_DIR" ]; then 
    ...
    echo "Successfully created and set ownership for $PHOENIXD_DATA_DIR"
else
    echo "Directory already exists: $PHOENIXD_DATA_DIR"; exit 1;
fi
...
if [ -d "$PHOENIXD_BACKUP_DIR" ]; then
	...
	tar xzvf $PHOENIXD_BACKUP_DIR/$BACKUP_FILE -C "$PHOENIXD_DATA_DIR"
else
